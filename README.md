...
`Syntax` refers to the node type specified with `--with-root-node` and `field` to a
...
If `--with-root-node` is not specified, _tsastgen_ will search for a declaration
named `Syntax`.
